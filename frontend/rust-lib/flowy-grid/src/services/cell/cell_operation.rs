...
pub fn apply_cell_data_changeset<C: ToString, T: AsRef<FieldRevision>>(
    ...
    let s = match field_type {
       ...
       FieldType::ChecklistSelect => ChecklistSelectTypeOption::from(field_rev).apply_changeset(changeset.into(), cell_rev),
    }?;
    ...
    let get_cell_data = || {
        ...
        let data = match t_field_type {
            ...
            FieldType::URL => field_rev
                ...
                .decode_cell_data(cell_data.into(), s_field_type, field_rev),
            FieldType::ChecklistSelect => field_rev.get_type_option_entry::<ChecklistSelectTypeOption>(field_type)?.decode_cell_data(cell_data.into(), s_field_type, field_rev),
        };
        Some(data)
    };
